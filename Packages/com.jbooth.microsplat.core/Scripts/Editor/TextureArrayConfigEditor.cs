         ...
         bool isCluster = false)
      {
         ...
         for (int i = 0; i < src.Count; ++i)
         {
            try
            {
               ...
               if (normalSAOTex != null) DestroyImmediate(normalSAOTex);
               if (smoothAOTex != null) DestroyImmediate(smoothAOTex);
               if (antiTileTex != null) DestroyImmediate(antiTileTex);
               if (emisTex != null) DestroyImmediate(emisTex);
               if (diffuseHeightTex != null) DestroyImmediate(diffuseHeightTex);
               if (traxDiffuseHeightTex != null) DestroyImmediate(traxDiffuseHeightTex);
               if (traxNormalSAOTex != null) DestroyImmediate(traxNormalSAOTex);
               if (splatTex != null) DestroyImmediate(splatTex);
